...
class WebHook {
  ...
  async deleteWebhook (url) {
    try {
      ...
      const result = await _this.axios.delete(endpoint, { data: obj })

      return result.data
    } catch (err) {
      console.log('Error in adapters/webhook/deleteWebHook(): ', err)
      throw err
    }
  }
  ...
  async waitUntilSuccess (url) {
    ...
    do {
      ...
      try {
        ...
        await sleep(this.webhookWaitPeriod)
      }
    } while (!success)
